   ...
   private NBTTagCompound getTagCompound(ItemStack stack, World world) {
      if(stack.stackTagCompound == null) {
         ...
         if(stack.stackTagCompound == null) {
            ...
            stack.stackTagCompound.setString("Engine", ((PartType)PartType.defaultEngines.get(EnumType.vehicle)).shortName);
         }
      }
      ...
      if(movingobjectposition == null) {
         ...
         if(movingobjectposition.typeOfHit == MovingObjectType.BLOCK) {
            ...
            if(this.type.placeableOnLand || block instanceof BlockLiquid) {
               if(!world.isRemote) {
                  world.spawnEntityInWorld(new EntityVehicle(world, (double)i + 0.5D, (double)j + 2.5D, (double)k + 0.5D, entityplayer, this.type, this.getData(itemstack, world)));
               }

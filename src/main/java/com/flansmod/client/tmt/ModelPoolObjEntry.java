...
public class ModelPoolObjEntry extends ModelPoolEntry {
   ...
   public ModelPoolObjEntry() {
      super.fileExtensions = new String[]{"obj"};
   }
               ...
               if(s.startsWith("g ")) {
                  ...
                  if(s.startsWith("v ")) {
                     ...
                     do {
                        ...
                        iNormal.add(Vec3.createVectorHelper((double)poly[0], (double)poly[1], (double)poly[2]));
                        normal[0] += poly[0];
                        normal[1] += poly[1];
                        normal[2] += poly[2];
                        if(d < verts.size()) {
                           i.add(((PositionTransformVertex)verts.get(d)).setTexturePosition(vToArr[0], vToArr[1]));
                           if(verts.get(d) instanceof PositionTransformVertex) {
                              ((PositionTransformVertex)verts.get(d)).addGroup(super.group);
                           }
                        }
                     ...
                     float var25 = MathHelper.sqrt_double((double)(normal[0] * normal[0] + normal[1] * normal[1] + normal[2] * normal[2]));
                     normal[0] /= var25;
                     normal[1] /= var25;
                     normal[2] /= var25;
                     ...
                     face.add(var28);
                     super.texture.addPoly(var28);
                  }
               }
            }
         }

         super.vertices = new PositionTransformVertex[verts.size()];

         int var22;
         for(var22 = 0; var22 < verts.size(); ++var22) {
            super.vertices[var22] = (PositionTransformVertex)verts.get(var22);
         }

         super.faces = new TexturedPolygon[face.size()];

         for(var22 = 0; var22 < face.size(); ++var22) {
            super.faces[var22] = (TexturedPolygon)face.get(var22);
         }
